...
class OmniDataset():
    def __init__(self, root_dir, batch_size, mode='train', H=0, W=0, correct_pose=[1, -1, -1], aabb_scale=None,
                 scale=None, offset=None, img_alpha=True, to_jt=True, have_img=True, preload_shuffle=True):
        ...
        #self.mask_data = self.mask_data.reshape(self.n_images, -1, 4)
        # breakpoint()
    ...
    def load_data(self, root_dir=None):
        ...
        assert npz_data is not None, f"dataset is not found at {root_dir}"
        ...
        image_folder = 'image'
        mask_folder = 'mask'
        image_list = sorted(os.listdir(os.path.join(self.root_dir, image_folder)))
        mask_list = sorted(os.listdir(os.path.join(self.root_dir, mask_folder)))

        for img_name in tqdm(image_list):
            img_path = os.path.join(self.root_dir, img_name)
            mask_path = os.path.join(self.root_dir, img_name)
            mat_ind = int(img_name.split('.')[0])
            camera_mat = npz_data['camera_mat_'+str(mat_ind)]
            world_mat = npz_data['world_mat_'+str(mat_ind)]
            scale_mat = npz_data['scale_mat_'+str(mat_ind)]
            if os.path.exists(img_path) and os.path.exists(mask_path):
                ...
                matrix = np.array(world_mat, np.float32)[:-1, :]
                self.transforms_gpu.append(self.matrix_nerf2ngp(matrix, scale_mat))
        ...
        metadata[4] = camera_mat[0, 2] / self.W
        #metadata[5] = json_data.get('cy', self.H / 2) / self.H
        metadata[5] = camera_mat[1, 2] / self.H
        fl_x, fl_y = camera_mat[0, 0], camera_mat[1, 1]
        self.focal_lengths.append([fl_x, fl_y])
        metadata[6] = fl_x
        metadata[7] = fl_y
    ...
    def matrix_nerf2ngp(self, matrix, scale_mat):
        matrix[:, 0] *= self.correct_pose[0]
        matrix[:, 1] *= self.correct_pose[1]
        matrix[:, 2] *= self.correct_pose[2]
        #matrix[:, 3] = matrix[:, 3] * scale + offset
        matrix[0, 3] = matrix[0, 3] * scale_mat[0, 0] + scale_mat[0, 3]
        matrix[1, 3] = matrix[1, 3] * scale_mat[1, 1] + scale_mat[1, 3]
        matrix[2, 3] = matrix[2, 3] * scale_mat[2, 2] + scale_mat[2, 3]
        # cycle
        matrix = matrix[[1, 2, 0]]
        return matrix

    def matrix_ngp2nerf(self, matrix, scale_mat):
        matrix = matrix[[2, 0, 1]]
        matrix[:, 0] *= self.correct_pose[0]
        matrix[:, 1] *= self.correct_pose[1]
        matrix[:, 2] *= self.correct_pose[2]
        #matrix[:, 3] = (matrix[:, 3] - offset) / scale
        matrix[0, 3] = (matrix[0, 3] - scale_mat[0, 3]) / scale_mat[0, 0]
        matrix[1, 3] = (matrix[1, 3] - scale_mat[1, 3]) / scale_mat[1, 1]
        matrix[2, 3] = (matrix[2, 3] - scale_mat[2, 3]) / scale_mat[2, 2]
        return matrix
...
class NerfDataset():
    ...
    def generate_rays_total(self, img_id, H, W):
        ...
        xy = jt.stack(jt.meshgrid((jt.linspace(0, H - 1, H) + 0.5) / H, (jt.linspace(0,
                                                                                     W - 1, W) + 0.5) / W),
                      dim=-1).permute(1, 0, 2).reshape(-1, 2)
        # assert H==W
